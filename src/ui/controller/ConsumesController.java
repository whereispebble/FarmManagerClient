 ...
 * </ul>
 *
 * <p>If any error occurs during the initialization, an error message is displayed and logged.</p>
 *
...
/**
 * Initializes the window components including the ComboBox, search field, buttons, 
 * and menu items. This method configures the basic UI elements and sets up their default states.
 ...
 * </ul>
 * 
 * <p>If an error occurs during the initialization, it is logged and displayed to the user.</p>
 *
...
/**
 * Initializes the table and sets up each column with appropriate cell factories, 
 * data bindings, and edit commit handlers. This method configures the table for displaying 
 * consume records, with each column bound to a specific property of the `ConsumesBean` model.
 * It also loads data for the columns (such as animal groups, products, and consume amounts)
 * and sets up event handlers to allow editing of the table's contents.
 * 
 * <p>This method performs the following actions:</p>
 * <ul>
 ...
 * </ul>
 * 
 * <p>If an error occurs while setting up any of the columns, it is logged and handled appropriately.</p>
 * 
...
/**
 * Sets up event handlers for all the UI components on the window.
 * This method configures actions for buttons, combo boxes, and table selections.
 */
...
/**
 * Handles changes in the combo box selection for the search criteria.
 * Updates the prompt text in the search field based on the selected search type.
 *
 ...
 * It creates a new `ConsumesBean` object with initial values, persists it via the backend, 
 * and adds it to the table for display. The newly added item is also edited in the column "Quantity".
 * 
...
/**
 * Handles the delete action. This method is invoked when the user clicks the "Delete" button.
 * It confirms the deletion of selected consumption items and removes them from both the backend and the table.
 * 
...
/**
 * Fetches and displays all consumes from the backend. If no consumes are found, 
 * it shows an informational alert. If any error occurs while fetching the data, 
...
/**
 * Handles exceptions by displaying the stack trace of the exception 
...
/**
 * Handles the print action. It generates a report based on the current table data 
 * and displays it in a JasperViewer window.
 * 
